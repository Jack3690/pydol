...
def wfc3_phot(flt_files, det='UVIS', filter='f814w',output_dir='.', drz_path='.',
                cat_name='', param_file=None,sharp_cut=0.15,
                crowd_cut=1.3):
    """
        Parameters
        ---------
        flt_files: list,
                    list of paths to HST WFC3 level 3 _flt.fits files
        det: str,
                WFC3 detector IR or UVIS
        filter: str,
                name of the WFC3 filter being processed
        output_dir: str,
    ...
    """
    if len(flt_files)<1:
        raise Exception("flt_files cannot be EMPTY")

    subprocess.run([f"wfc3mask {drz_path}.fits"], shell=True)
    ...
    if param_file is None or not os.path.exists(param_file) :
        ...
        edit_params = True
        if det=='UVIS':
            param_file = param_dir_default + '/wfc3_uvis_dolphot.param'
        elif det=='IR':
            param_file = param_dir_default + '/wfc3_ir_dolphot.param'
        else:
            raise Exception(f"""Incorrect input det = {det}. 
                            Accepted inputs 'IR' or 'UVIS'""")
    ...
    for f in exps:
        if not os.path.exists(f"{f}/data.chip1.sky.fits") or not os.path.exists(f"{f}/data.chip2.sky.fits") :
            ...
            if det=='UVIS':
                ...
                out = subprocess.run([f"calcsky {f}/data.chip1 10 25 2 2.25 2.00"]
                                    , shell=True, capture_output=True)
                
                out = subprocess.run([f"calcsky {f}/data.chip2 10 25 2 2.25 2.00"]
                                    , shell=True, capture_output=True)

    if edit_params:
      # Preparing Parameter file DOLPHOT WFC3
      ...
      with open(f"{output_dir}/wfc3_dolphot_{out_id}.param", 'w', encoding='utf-8') as f:
          f.writelines(dat)
      param_file = f"{output_dir}/wfc3_dolphot_{out_id}.param"
    if not os.path.exists(f"{output_dir}/{out_id}_photometry.fits"):
        # Running DOLPHOT WFC3
    ...
    print('WFC3 Stellar Photometry Completed!')
