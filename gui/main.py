...
class FITSCanvas(FigureCanvasQTAgg):
    ...
    def plot_fits(self, data, wcs=None, title=None):
        ...
        data = np.nan_to_num(data, nan=0.0, posinf=np.nanmax(data) if np.isfinite(np.nanmax(data)) else 0.0, neginf=0.0)
        positive = data[data > 0]

        if positive.size == 0:
            self.ax.text(0.5, 0.5, "No positive image values for log scaling.",
        ...
        vmin = positive.min()
        vmax = positive.max() if positive.max() > vmin else vmin * 10
...
class StellarPopulationGUI(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle(
            "CMD Explorer - Stellar Population Analysis Tool"
        )
